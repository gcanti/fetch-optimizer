...
function optimize(dependencies, fetchers, initPromise) {
...
var Poset = (function () {
  ...
  Poset.prototype.addNode = function addNode() {
    var _this = this;
    ...
    nodes.forEach(function (node) {
      return _this.nodes[node] = _this.nodes[node] || {};
    });
    return this;
  };
  ...
  Poset.prototype.addEdge = function addEdge(from, to) {
    ...
    this.addNode(to);
    [].concat(from).forEach(function (node) {
      _this2.addNode(node);
      _this2.nodes[node][to] = 1;
    });
    return this;
  };
  ...
  Poset.prototype.getUpsetPoset = function getUpsetPoset(nodes) {
    ...
    var addNodes = function addNodes(nodes) {
      for (var node in nodes) {
        if (_this3.nodes.hasOwnProperty(node)) {
          if (!ret.hasOwnProperty(node)) {
            var edges = _this3.getEdges(node);
            ret[node] = edges;
            addNodes(edges);
          }
        } else if (process.env.NODE_ENV !== 'production') {
